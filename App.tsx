      ...
      {detailItem && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-3xl bg-slate-900/60 animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <header className="px-8 py-6 border-b border-slate-50 dark:border-slate-800 flex justify-between items-center">
              <span className="px-4 py-1 bg-indigo-600 text-white text-[10px] font-black rounded-full uppercase tracking-widest">{detailItem.category}</span>
              <button onClick={() => setDetailItem(null)} className="p-2 bg-slate-50 dark:bg-slate-800 rounded-full hover:bg-rose-500 hover:text-white transition-all">
                <X className="w-5 h-5"/>
              </button>
            </header>

            <div className="flex-1 overflow-y-auto hide-scrollbar p-10">
              <h2 className="text-2xl md:text-3xl font-black mb-8 leading-tight">{detailItem.title}</h2>
              <div className="bg-indigo-50/50 dark:bg-slate-800/40 p-8 rounded-3xl border border-indigo-100 dark:border-slate-800 mb-8">
                <p className="text-lg font-medium text-slate-700 dark:text-slate-200 leading-relaxed italic">
                  {detailItem.summary}
                </p>
              </div>
              {detailItem.url && (
                <a href={detailItem.url} target="_blank" rel="noreferrer" className="w-full flex items-center justify-center gap-3 px-8 py-5 bg-slate-900 dark:bg-indigo-600 text-white rounded-2xl font-black text-sm hover:opacity-90 transition-all">
                  <Newspaper className="w-5 h-5"/> 阅读原文完整报道
                </a>
              )}
            </div>
            
            <footer className="px-8 py-6 bg-slate-50 dark:bg-slate-800/20 border-t border-slate-100 dark:border-slate-800 flex gap-4">
               <button onClick={() => alert('已复制分享链接')} className="flex-1 py-4 bg-white dark:bg-slate-900 text-slate-500 rounded-xl font-black text-[10px] flex items-center justify-center gap-2 border border-slate-200 dark:border-slate-700">
                 <Share2 className="w-3 h-3" /> 分享摘要
               </button>
        ...
        <div className="max-w-7xl mx-auto px-6 h-full flex items-center justify-between">
          ...
          <div className="hidden md:flex flex-1 max-w-xs mx-8">
            <div className="relative w-full">
              ...
              <input 
                ...
                className="w-full bg-slate-100 dark:bg-slate-900 border-none rounded-xl py-2 pl-9 pr-4 text-[11px] font-medium outline-none"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800">
              ...
              <span className="text-[10px] font-black text-slate-600">{selectedDate}</span>
            </div>
            <button onClick={() => loadNews(selectedDate)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg">
        ...
        {loading ? <HeroSkeleton /> : report && (
          <div className="space-y-12">
            <section className="grid grid-cols-1 lg:grid-cols-4 gap-8">
              <div className="lg:col-span-3 bg-slate-900 text-white rounded-[3rem] p-10 md:p-12 relative overflow-hidden flex flex-col justify-center min-h-[300px]">
                <BarChart3 className="absolute -bottom-10 -right-10 opacity-5 w-64 h-64" />
                <div className="relative z-10 space-y-6">
                  <div className="inline-block px-3 py-1 bg-white/10 rounded-full text-[9px] font-black tracking-widest text-indigo-400">今日头条</div>
                  <h3 className="text-3xl md:text-5xl font-black leading-tight max-w-4xl">{report.headline}</h3>
                  <p className="text-slate-400 text-sm max-w-xl font-medium">{report.trendAnalysis}</p>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-900 rounded-[3rem] p-8 border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col">
                <h3 className="text-[10px] font-black mb-6 text-slate-400 tracking-widest uppercase flex items-center gap-2">
                  <Activity className="w-3.5 h-3.5 text-indigo-600" /> 访问趋势 (7D)
                </h3>
                <TrafficChart key={refreshTrigger} />
                <div className="mt-auto pt-6 space-y-4">
                  {categoryHeat.slice(0, 3).map(cat => (
                    <div key={cat.name} className="flex items-center justify-between">
                       <span className="text-[9px] font-black text-slate-500">{cat.name}</span>
                       <div className="flex-1 h-1 mx-3 bg-slate-50 dark:bg-slate-800 rounded-full overflow-hidden">
                         <div className="h-full bg-indigo-500" style={{ width: `${cat.percentage}%` }}></div>
                       </div>
            ...
            <div className="flex flex-col lg:flex-row gap-12">
                ...
                <div className="sticky top-32 space-y-2">
                  ...
                  {categories.map(cat => (
                    <button 
                      ...
                      className={`w-full text-left px-5 py-3 rounded-2xl text-[10px] font-black transition-all ${activeCategory === cat ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white dark:bg-slate-900 text-slate-500 border border-slate-100 dark:border-slate-800 hover:border-indigo-200'}`}
                    >
                      {cat}
                    </button>
                  ))}
              ...
              <div className="flex-1 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-6">
                ...
                {filteredNews.length === 0 && (
                  <div className="col-span-full py-24 text-center text-slate-400 text-xs font-bold uppercase tracking-widest">暂无匹配资讯</div>
                )}
              </div>
            </div>
          </div>
